...
class SpecReporter extends StreamReporter {
  ...
  withHarness(harness) {
    harness.once(RunnableEvents.FINISHED, () => {
      ...
      const summary = [
        this._effects(`Passed: ${harness.summary.passed.toLocaleString()}`, 'green'),
        this._effects(`Failed: ${harness.summary.failed.toLocaleString()}`, 'red'),
        this._effects(`Skipped: ${harness.summary.skipped.toLocaleString()}`, 'cyan'),
        this._effects(`Duration: ${harness.summary.duration.toLocaleString()}ms`, 'white'),
      ].join(', ');
