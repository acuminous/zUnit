...
class JUnitReporter extends StreamReporter {
  ...
  _writeTestSuites(node) {
    this._writeln(`<testsuites name="${node.name}" tests="${node.tests}" failures="${node.failures}" time="${node.duration / 1000}">`);
  ...
  _writeTestSuite(node, children) {
    this._writeln(`  <testsuite name="${node.name}" tests="${node.tests}" failures="${node.failures}" skipped="${node.skips}" time="${node.duration / 1000}">`);
