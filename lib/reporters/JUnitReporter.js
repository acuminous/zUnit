...
class JUnitReporter extends StreamReporter {
  ...
  _writeTestSuites(node) {
    this._writeln(`<testsuites name="${node.name}" tests="${node.stats.tested}" failures="${node.stats.failed}" time="${node.duration / 1000}">`);
  ...
  _writeTestSuite(node, children) {
    this._writeln(`  <testsuite name="${node.name}" tests="${node.stats.tested}" failures="${node.stats.failed}" skipped="${node.stats.skipped}" time="${node.duration / 1000}">`);
