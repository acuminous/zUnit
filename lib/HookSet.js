const appendAll = require('./utils/appendAll');

class HookSet {

  constructor() {
    this._befores = [];
    this._afters = [];
    this._started = false;
  }

  _finalise(parent) {
    const hookSet = new HookSet();
    hookSet._parent = parent;
    hookSet._befores = this._befores.map((hook) => hook._finalise(parent));
    hookSet._afters = this._afters.map((hook) => hook._finalise(parent));
    return hookSet;
  }

  addBefores(...additions) {
    const befores = additions.map((hook) => hook.toBefore());
    this._befores = appendAll(this._befores, befores);
    return this;
  }

  addAfters(...additions) {
    const afters = additions.map((hook) => hook.toAfter());
    this._afters = appendAll(this._afters, afters);
    return this;
  }

  async runBefores() {
    this._started = true;
    for (let i = 0; i < this._befores.length; i++) {
      await this._befores[i].run();
      if (this._parent.skipped) break;
    }
  }

  async runAfters() {
    if (!this._started) return;
    for (let i = 0; i < this._afters.length; i++) {
      await this._afters[i].run();
    }
  }
}

module.exports = HookSet;
