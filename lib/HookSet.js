...
class HookSet {
  ...
  async runBefores(runtimeOptions, inheritedOptions) {
    this._started = true;
    for (let i = 0; i < this._befores.length; i++) {
      await this._befores[i].run(runtimeOptions, inheritedOptions);
      if (this._parent.skipped) break;
    }
  }

  async runAfters(runtimeOptions, inheritedOptions) {
    if (!this._started) return;
    for (let i = 0; i < this._afters.length; i++) {
      await this._afters[i].run(runtimeOptions, inheritedOptions);
    }
  }
}
