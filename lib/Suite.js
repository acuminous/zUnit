...
class Suite extends EventEmitter {
  ...
  async _run(reporter, options) {
    ...
    try {
      ...
      for (let i = 0; i < this._runnables.length; i++) {
        ...
        const runnableOptions = config.skip || aborting ? Object.assign({}, options, { skip: true }) : options;
        await runnable._run(suiteReporter, runnableOptions);
  ...
  _finish(config) {
    ...
    this.result = config.skip ? Events.SUITE_SKIPPED : this._aggregateReporter.result;
