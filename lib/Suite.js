...
class Suite extends Testable {
  ...
  async run(reporter, runtimeOptions = {}, ancestorOptions = {}, force = true) {

    const suiteReporter = reporter.withSuite(this);

    this._start();

    const inheritableOptions = this._getInheritableOptions(ancestorOptions);
  ...
  _getInheritableOptions(ancestorOptions) {
    // eslint-disable-next-line no-unused-vars
    const { exclusive, ...inheritableOptions } = Object.assign({}, defaults, ancestorOptions, this._options);
  ...
  async _runAll(testables, reporter, runtimeOptions, inheritableOptions, runOptions, force) {
    try {
      if (this._shouldRunHooks(runOptions, testables)) await this._suiteHooks.runBefores(runtimeOptions, inheritableOptions);
      ...
      this._run = true;
      if (this._shouldRunHooks(runOptions, testables)) await this._suiteHooks.runAfters(runtimeOptions, inheritableOptions);
    }
  }
