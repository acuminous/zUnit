...
function findFiles(options = {}) {
  ...
  if (!directory) throw new Error('directory is a required option');

  return fs.readdirSync(directory).map(toFullPath).filter(byMatchingFilePath).reduce(toFlattenedFilePaths, []).sort(alphabetically);
  ...
  function toFlattenedFilePaths(filePaths, filePath) {
    const additions = fs.statSync(filePath).isDirectory() ? findFiles({ ...options, directory: filePath }, filePaths) : filePath;
