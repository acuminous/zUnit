...
function before(...args) {
  const hook = createHook({ name: 'before', ...getHookParameters(...args) });
  currentSuite().before(hook);
}

function beforeEach(...args) {
  const hook = createHook({ name: 'beforeEach', ...getHookParameters(...args) });
  currentSuite().beforeEach(hook);
}

function afterEach(...args) {
  const hook = createHook({ name: 'afterEach', ...getHookParameters(...args) });
  currentSuite().afterEach(hook);
}

function after(...args) {
  const hook = createHook({ name: 'after', ...getHookParameters(...args) });
  currentSuite().after(hook);
}

function createHook({ name, fn, options }) {
